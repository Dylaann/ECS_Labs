	                      ...
	                      entityx::EntityManager& entities) const
{
	...
	sf::Vector2f pplayerPos = playerPos->m_position;
	sf::Vector2f aaiPos = aiPos->m_position;
																     ...
																     entityx::EntityManager& entities) 
{
	...
	for (int i = 0; i < size; i++)
	{
		if (Math::lineIntersectsCircle(m_ahead, m_halfAhead, m_obstacles[i]) && 
			(mostThreatening.getRadius() == 0 || Math::distance(aiPos->m_position, m_obstacles.at(i).getPosition()) < Math::distance(aiPos->m_position, mostThreatening.getPosition())))
		{
