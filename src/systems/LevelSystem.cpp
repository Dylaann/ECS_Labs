...
void LevelSystem::receive(const EvInit& e)
{
   ...
   //BackgroundCreator(e.m_level.m_background.m_fileName).create(m_entityManager.create());
	entityx::Entity backgroundEntity = m_entityManager.create();
	BackgroundCreator(e.m_level.m_background.m_fileName).create(backgroundEntity);
   ...
   //Create nodes
   for (nodeData const &waypoint : e.m_level.m_nodes)
   {
	   NodeCreator(waypoint.m_type, waypoint.m_position, waypoint.radius).create(m_entityManager.create());
   }
